...
func NewFakeDb(options ...TestOption) *FakeDb {
...
func (db *FakeDb) getTasks(f func(any) error) error {
    if !db.opts.isAvailable {
        ...
        return fmt.Errorf("unexpected error: couldn't access the database")
    }
...
func (db *FakeDb) updateTask(id uint64, description string, dueDate time.Time, done bool) error {
    if !db.opts.isAvailable {
        return fmt.Errorf("couldn't access the database")
    }
